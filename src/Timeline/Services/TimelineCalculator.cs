...
{
    /// <summary>Порог длительности в днях: если интервал не превышает это значение, верхний ряд — годы, иначе кварталы.</summary>
    private const int QuarterDurationDaysThreshold = 92;
    ...
    private static int WeekGroupKey(TimelineDivision d) => IsoWeekCalculator.IsoWeekKey(d.Start);
    ...
    private static TimelineScale ResolveUpperScale(TimelineScale lowerScale, DateOnly start, DateOnly end)
    {
        return lowerScale switch
        {
            ...
            TimelineScale.Months => DurationDays(start, end) <= QuarterDurationDaysThreshold ? TimelineScale.Years : TimelineScale.Quarters,
    ...
    private static int UpperGroupKey(DateOnly d, TimelineScale upperScale)
    {
        ...
        {
            TimelineScale.Weeks => IsoWeekCalculator.IsoWeekKey(d),
