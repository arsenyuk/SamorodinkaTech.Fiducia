...
{
    /// <summary>Ожидаемая длина строки цифр в формате GeneralizedTime (YYYYMMDDHHmmss).</summary>
    private const int GeneralizedTimeDigitLength = 14;
    ...
    private static DateTime? ParseCreatedAt(SearchResultEntry entry)
    {
        ...
        if (digits.Length < GeneralizedTimeDigitLength) return null;
