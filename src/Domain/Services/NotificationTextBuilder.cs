...
public class NotificationTextBuilder
{
    ...
    {
        var formText = meeting.MeetingFormId switch
        {
            "ZAOCHN" => "заочное",
    ...
    /// <summary>
    /// Рассылка по сбору предложений кандидатур на должность Председателя СД.
    /// </summary>
    /// <param name="legalEntityName">Полное наименование юридического лица.</param>
    public (string Title, string Body) BuildChairmanNomination(string legalEntityName)
    {
        return (
            $"Сбор предложений кандидатур — Председатель СД {legalEntityName}",
            $"Уважаемый член совета директоров!\n\n"
            + $"В рамках подготовки к первому (организационному) заседанию совета директоров "
            + $"{legalEntityName} просим вас направить предложения по кандидатурам "
            + $"на должность Председателя совета директоров.\n\n"
    ...
    /// <summary>
    /// Рассылка по сбору предложений кандидатур на должность Заместителя председателя СД.
    /// </summary>
    /// <param name="legalEntityName">Полное наименование юридического лица.</param>
    public (string Title, string Body) BuildDeputyChairmanNomination(string legalEntityName)
    {
        return (
            ...
            + $"В рамках подготовки к первому (организационному) заседанию совета директоров "
            + $"{legalEntityName} просим вас направить предложения по кандидатурам "
            + $"на должность Заместителя председателя совета директоров.\n\n"
            + $"Предложения принимаются до даты, установленной внутренними документами Организации.\n\n"
            + $"С уважением,\n"
            + $"Секретарь совета директоров {legalEntityName}"
        );
    ...
    /// <summary>
    /// Уведомление кандидату о необходимости подписать согласие на должность Председателя СД.
    /// </summary>
    /// <param name="legalEntityName">Полное наименование юридического лица.</param>
    /// <param name="candidateName">ФИО кандидата.</param>
    public (string Title, string Body) BuildChairmanConsent(string legalEntityName, string candidateName)
    {
        return (
            $"Согласие на должность Председателя СД {legalEntityName}",
            $"Уважаемый {candidateName}!\n\n"
            + $"Ваша кандидатура предложена на должность Председателя совета директоров "
            + $"{legalEntityName}. Для включения в бюллетень голосования на первом "
            + $"(организационном) заседании совета директоров необходимо подписать "
            + $"согласие на выдвижение.\n\n"
            + $"Подписание согласия равнозначно собственноручной подписи "
    ...
    /// <summary>
    /// Уведомление кандидату о необходимости подписать согласие на должность Заместителя председателя СД.
    /// </summary>
    /// <param name="legalEntityName">Полное наименование юридического лица.</param>
    /// <param name="candidateName">ФИО кандидата.</param>
    public (string Title, string Body) BuildDeputyChairmanConsent(string legalEntityName, string candidateName)
    {
        return (
            $"Согласие на должность Заместителя председателя СД {legalEntityName}",
            $"Уважаемый {candidateName}!\n\n"
            + $"Ваша кандидатура предложена на должность Заместителя председателя совета "
            + $"директоров {legalEntityName}. Для включения в бюллетень голосования на "
            + $"первом (организационном) заседании совета директоров необходимо подписать "
            + $"согласие на выдвижение.\n\n"
            + $"Подписание согласия равнозначно собственноручной подписи "
            + $"в соответствии с Федеральным законом № 63-ФЗ «Об электронной подписи».\n\n"
            + $"С уважением,\n"
