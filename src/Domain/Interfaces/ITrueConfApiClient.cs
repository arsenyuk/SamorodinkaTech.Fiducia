...
public interface ITrueConfApiClient
{
    ...
    /// <summary>
    /// Возвращает информацию о сервере TrueConf (версия, имя, платформа).
    /// </summary>
    /// <param name="cancellationToken">Токен отмены.</param>
    /// <returns>Данные сервера или null, если не удалось получить.</returns>
    Task<TrueConfServerInfo?> GetServerInfoAsync(
        CancellationToken cancellationToken = default);
}
