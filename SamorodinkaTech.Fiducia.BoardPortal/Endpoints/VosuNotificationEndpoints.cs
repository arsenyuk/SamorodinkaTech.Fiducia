...
public static class VosuNotificationEndpoints
{
    public static void MapVosuNotificationEndpoints(this WebApplication app)
    {
            ...
            try
            {
                ...
                foreach (var participant in participants)
                {
                    ...
                    var docxData = new VosuNotificationData
                    {
                        ...
                        IsAgendaChange = request.IsAgendaChange
                    };
        ...
        // ═══════════════════════════════════════════════════════════
        // ООСУ — уведомление о проведении очередного собрания
        // ═══════════════════════════════════════════════════════════

        var oosuNotifications = app.MapGroup("/api/oosu-notifications")
            .RequireAuthorization()
            .WithTags("OOSU Notifications");

        oosuNotifications.MapPost("/send", async (
            OosuNotificationRequest request,
            IDbContextFactory<FiduciaDbContext> dbFactory,
            INotificationService notificationService,
            NotificationTextBuilder textBuilder,
            ...
            HttpContext http) =>
        {
            ...
            try
            {
                ...
                logger.LogInformation("Отправлено уведомлений ООСУ: {Count}, файлов DOCX: {FileCount}",
                    sentCount, fileIds.Count);

                return Results.Ok(new { sentCount, fileCount = fileIds.Count, meetingId = meeting.Id });
            }
            catch (Exception ex)
            {
                logger.LogWarning(ex, "Ошибка отправки уведомлений ООСУ: {Error}",
                    ExceptionFlattener.Unwrap(ex));
                return Results.BadRequest(new { error = ExceptionFlattener.Unwrap(ex) });
            }
        });

        oosuNotifications.MapGet("/preview/{meetingId:guid}", async (
            Guid meetingId,
            IDbContextFactory<FiduciaDbContext> dbFactory,
            HttpContext http) =>
        {
...
/// <summary>
/// Запрос на отправку уведомлений ООСУ.
/// </summary>
public class OosuNotificationRequest
{
    /// <summary>Идентификатор собрания.</summary>
    public Guid MeetingId { get; init; }
