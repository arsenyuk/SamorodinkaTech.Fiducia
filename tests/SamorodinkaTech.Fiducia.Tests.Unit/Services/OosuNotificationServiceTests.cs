...
public class OosuNotificationServiceTests : IDisposable
{
    private readonly Infrastructure.Persistence.FiduciaDbContext _ctx;
    private readonly Mock<INotificationService> _notificationServiceMock;
    private readonly Mock<IOosuNotificationDocxGenerator> _docxGeneratorMock;
    private readonly Mock<IFileStorage> _fileStorageMock;
    private readonly OosuNotificationService _sut;

    public OosuNotificationServiceTests()
    {
        var (factory, _) = WriteServiceTestBase.CreateMockFactory(out _ctx);
        _notificationServiceMock = new Mock<INotificationService>();
        _docxGeneratorMock = new Mock<IOosuNotificationDocxGenerator>();
        _fileStorageMock = new Mock<IFileStorage>();
        var textBuilder = new NotificationTextBuilder(_ctx);
        var logger = Mock.Of<ILogger<OosuNotificationService>>();

        _docxGeneratorMock
            .Setup(g => g.GenerateAsync(It.IsAny<OosuNotificationData>(), It.IsAny<CancellationToken>()))
        ...
        _fileStorageMock
            ...
            .ReturnsAsync("storage-key-456");

        _sut = new OosuNotificationService(factory, _notificationServiceMock.Object, textBuilder,
    ...
    public async Task SendAsync_ValidMeeting_CreatesNotifications()
    {
        ...
        var model = new OosuNotificationSendModel
        {
            ...
            LegalEntityId = le.Id,
            UserId = user.Id,
            MeetingDate = new DateOnly(2025, 10, 1),
            MeetingStartTime = new TimeOnly(14, 0),
            MeetingVenue = "г. Москва, пл. ООСУ, 5",
            FinancialYear = 2024,
            CeoName = "Иванов И.И."
        };
        ...
        updatedMeeting!.GosaWindowStart.Should().Be(new DateOnly(2025, 10, 1));
        updatedMeeting.MeetingVenue.Should().Be("г. Москва, пл. ООСУ, 5");

        _ctx.Files.Count().Should().Be(1);
        _ctx.OsaMeetingFiles.Count().Should().Be(1);

        _docxGeneratorMock.Verify(g =>
            g.GenerateAsync(It.IsAny<OosuNotificationData>(), It.IsAny<CancellationToken>()),
            Times.Once);
    }
