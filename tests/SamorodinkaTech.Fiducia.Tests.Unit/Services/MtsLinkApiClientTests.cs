...
public class MtsLinkApiClientTests
{
    ...
    /// <summary>
    /// Получение участников существующей сессии возвращает список зарегистрированных.
    /// </summary>
    [Fact]
    public async Task GetParticipationsAsync_ShouldReturnRegisteredParticipants()
    {
        var meeting = await _client.CreateMeetingAsync(new CreateMtsLinkMeetingRequest
        {
            Name = "Заседание СД",
        ...
        await _client.RegisterParticipantAsync(meeting.Id,
            new RegisterMtsLinkParticipantRequest { Email = "dir1@company.ru", Name = "Иван", SecondName = "Иванов" });
        await _client.RegisterParticipantAsync(meeting.Id,
            new RegisterMtsLinkParticipantRequest { Email = "dir2@company.ru", Name = "Пётр", SecondName = "Петров" });

        var result = await _client.GetParticipationsAsync(meeting.Id);

        result.Should().HaveCount(2);
        result[0].Link.Should().NotBeNullOrEmpty();
        result[1].Link.Should().NotBeNullOrEmpty();
    }

    /// <summary>
    /// Получение участников несуществующей сессии выбрасывает KeyNotFoundException.
    /// </summary>
    [Fact]
    public async Task GetParticipationsAsync_NonExistingSession_ShouldThrow()
    {
        var act = () => _client.GetParticipationsAsync(999);
    ...
    public async Task SimulateFailure_ShouldThrowOnAllOperations()
    {
        ...
        var participationsAct = () => _client.GetParticipationsAsync(1);
        await participationsAct.Should().ThrowAsync<HttpRequestException>();
    }
}
