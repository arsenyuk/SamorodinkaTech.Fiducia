...
| ▼ВР1 | Фаза 1 завершена | — | — | FS | FS | СД1 | Все задачи Фазы 1 выполнены |

### Фаза 2: Проверка отчётности (февраль – апрель)

| ID | Задача | Исполнитель | Длит. | Начало | Конец | Предшественники | Примечание |
|----|--------|------------|-------------|--------|-------|-----------------|------------|
| Р1 | Передача документов ревизору | Гендиректор | 1 раб. дн. | FS + 1 раб. дн. | FS + 1 | ▼ВР1 | Параллельно с А2 |
...
| А2 | Передача документов аудитору | Гендиректор | 1 раб. дн. | FS + 1 раб. дн. | FS + 1 | ▼ВР1 | Параллельно с Р1 |
...
| ▼ВР2 | Фаза 2 завершена | — | — | FS | FS | А4, Р3 | Заключения ревизора и аудитора получены |

### Фаза 3: Решение СД о созыве и подготовка списка (март – апрель)

| ID | Задача | Исполнитель | Длит. | Начало | Конец | Предшественники | Примечание |
|----|--------|------------|-------------|--------|-------|-----------------|------------|
| СД2 | Заседание СД: утверждение годового отчёта | Совет директоров | 1 раб. дн. | FS + 1 раб. дн. | FS + 1 | ▼ВР2 | Годовой отчёт предварительно утверждается СД перед ГОСА |
| СД3 | Заседание СД: решение о созыве ГОСА | Совет директоров | 1 раб. дн. | FS + 1 раб. дн. | FS + 1 | ▼ВР2 | Дата, повестка, форма, дата фиксации списка. После получения аудиторского заключения |
...
| ▼ВР3 | Фаза 3 завершена | — | — | FS | FS | РГ4 | Список акционеров проверен, ГОСА назначено |

### Фаза 4: Подготовка и уведомление (апрель – май)

| ID | Задача | Исполнитель | Длит. | Начало | Конец | Предшественники | Примечание |
|----|--------|------------|-------------|--------|-------|-----------------|------------|
| П1 | Формирование повестки и проектов решений | Совет директоров | 2 раб. дн. | FS + 1 раб. дн. | FS + 1 | ▼ВР3 | [С3](#c3). На основе предложений акционеров (СД1) и обязательных вопросов |
...
| ⚡КВ1 | Уведомление акционеров — дедлайн | — | — | 24.04 | 24.04 | — | Ст. 52 208-ФЗ: не позднее 21 кал. дн. до ГОСА |

| ▼ВР4 | Фаза 4 завершена | — | — | FS | FS | Д1 | Уведомления разосланы, материалы доступны |
...
| ⚡КВ2 | Раскрытие протокола — дедлайн (ПАО) | — | — | 22.05 | 22.05 | — | ЦБ №714-П: не позднее 4 раб. дн. после ГОСА |
| ⚡КВ3 | Подача Р13014 — дедлайн | — | — | 27.05 | 27.05 | — | Ст. 5 129-ФЗ: не более 7 раб. дн. после заседания |
