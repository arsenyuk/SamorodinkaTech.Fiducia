...
> **Связанные документы:** [Схема (ER-диаграмма)](database-schema.md) | [Индекс документации](database.md)
...
Индекс: `ix_board_participant_legal_entity` по `legal_entity_id`.

### board_participant_company

Сведения об ЮЛ участника. SCD Type 2: каждая версия — отдельная запись. Только для UL-участников (`participant_type = 'UL'`).
...
Индексы: `ix_bpco_participant` по `participant_id`, `ix_bpco_active` по `(participant_id, is_active) WHERE is_active = true`.
...
Записи сведений участника (информационирование об изменении). При электронном источнике — автоприменение к `identity_documents`, `person` и `board_participant_company`. При бумажном — статус `pending` → `approved`/`rejected` через рассмотрение ГД.
