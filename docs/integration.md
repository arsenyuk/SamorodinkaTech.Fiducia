...
`docker-compose.ldap.yml` — OpenLDAP с seed-данными (6 директоров ООО «Фидуция» + администратор). **Не работает на Apple Silicon** — требуется нативный x86_64 Linux.
...
> **Тип API:** официальный API СПАРК — SOAP/XML с более чем 50 методами. В проекте используется REST/JSON-слой (предоставляется Интерфакс опционально или через прокси-сервис). При интеграции через SOAP потребуется генерация клиента из WSDL и замена `SparkApiClient`.

---

## Лицензионные уровни и доступность данных

СПАРК не публикует фиксированные тарифы — стоимость зависит от набора данных и объёма запросов. Ниже — ориентировочная разбивка по уровням доступа.

| Уровень | Что входит | Доступность API |
|---------|-----------|-----------------|
| **Базовый** (веб-доступ) | Регистрационные данные, руководитель, статус, адрес | ❌ API не предоставляется |
| **Стандарт** (+ API) | + Карточка компании (`/companies`), руководители (`/managers`), выписки ЕГРЮЛ, бух. отчётность, арбитраж | ✅ Базовые методы API |
| **Расширенный** | + Учредители/совладельцы (`/founders`), структура собственников, аффилированность, бенефициары, совет директоров | ✅ Расширенные методы API |
| **Мониторинг** | + Мониторинг изменений по списку компаний, уведомления о событиях | ✅ Webhook/API |

### Какие данные на каком уровне

| Данные | Базовый | Стандарт | Расширенный |
|--------|---------|----------|-------------|
| Карточка компании (ИНН, ОГРН, ОКОПФ, адрес, статус) | ✅ | ✅ | ✅ |
| Руководитель (ФИО, должность, ИНН, дата назначения) | ✅ | ✅ | ✅ |
| Бухгалтерская отчётность | ❌ | ✅ | ✅ |
| Выписки ЕГРЮЛ/ЕГРИП | ❌ | ✅ | ✅ |
| **Учредители/участники** (ФИО/название, ИНН, доля) | ❌ | ❌ | ✅ |
| Совладельцы и структура собственников | ❌ | ❌ | ✅ |
| Аффилированные лица | ❌ | ❌ | ✅ |
| Конечные бенефициары | ❌ | ❌ | ✅ |
| Совет директоров (состав) | ❌ | ❌ | ✅ |
| Финансовые индексы и скоринг | ❌ | ❌ | ✅ |
| Арбитражные дела, ФССП | ❌ | ✅ | ✅ |
| Мониторинг изменений | ❌ | ❌ | ✅ |

> **Для нашей задачи (получение учредителей ООО):** требуется тариф уровня «Расширенный». Базового API-тарифа недостаточно — учредители не входят в стандартную поставку.

### Ограничения API

| Параметр | Значение |
|----------|----------|
| Пропускная способность | ~100 запросов/сек на клиента |
| Суточный лимит | До 25 млн запросов (на пул клиентов) |
| Формат | SOAP/XML (основной), REST/JSON (опционально) |
| Методов API | > 50 |

> **Важно для `founders`:** endpoint запрашивается **только для ООО**. Для АО (ПАО/НАО) состав акционеров ведёт регистратор — эти данные отсутствуют в ЕГРЮЛ и, следовательно, в СПАРК. Что СПАРК может вернуть для АО — это первоначальные учредители при регистрации (исторический срез, не актуальный состав акционеров). Код проекта блокирует вызов `GetFoundersAsync` и отображение секции «Учредители» для `OrgType` ≠ `LLC`.
...
| `GET` | `/api/v1/companies/{inn}/founders` | Учредители/участники ООО: ФИО/название, ИНН, размер доли (₽ и %) |

Из списка руководителей клиент возвращает первого (генерального директора).

> **Область применения `founders`:** endpoint актуален только для **ООО** (ОКОПФ 12300). Для АО (ПАО, НАО) реестр акционеров ведёт лицензированный регистратор, и эти данные отсутствуют как в ЕГРЮЛ, так и в СПАРК. В коде вызов `GetFoundersAsync` и отображение секции «Учредители» заблокированы для не-ООО (`_orgType != OrgType.LLC`).

> **Требования к тарифу:** `founders` доступен только на уровне «Расширенный».
...
### Ежедневный запрос данных ООО из СПАРК

**Периодичность**: раз в сутки (рекомендуется ночной запуск, например 03:00 МСК).

**Назначение**: поддержание кэша `ext_spark_company`, `ext_spark_manager` и `ext_spark_founder` в актуальном состоянии. Данные в ЕГРЮЛ меняются (смена руководителя, состава участников, реорганизация) — без периодического обновления кэш устаревает и Board Portal показывает неактуальный состав участников ООО.

**Что делает задача**:
1. Выбирает из `legal_entities` все записи с ИНН, у которых `okopf.code = '12300'` (ООО).
2. Для каждой вызывает `ISparkApiClient.GetCompanyByInnAsync(inn)` → upsert в `ext_spark_company`.
3. Вызывает `ISparkApiClient.GetManagerAsync(inn)` → upsert в `ext_spark_manager`.
4. Вызывает `ISparkApiClient.GetFoundersAsync(inn)` → replace (удалить старые, вставить новые) в `ext_spark_founder`.
5. Логирует результат: количество обновлённых записей, ошибки (включая 403 — недостаточная лицензия).

**Требования к тарифу**: для п. 4 необходим уровень «Расширенный». Если тариф не включает учредителей, п. 4 пропускается (с предупреждением в лог), пп. 1–3 выполняются.

**Реализация**: фоновая задача через `IHostedService` / `BackgroundService` в AdminConsole или отдельный консольный процесс, запускаемый по cron. Не требует UI — чисто серверная операция.

> **Текущий статус**: не реализовано. Кэш обновляется только при ручном нажатии кнопки «СПАРК» в Admin Console → Юридические лица.
...
Реестр дисквалифицированных лиц ведётся Федеральной налоговой службой в соответствии с постановлением Правительства РФ от 11.11.2002 № 805. РДЛ используется для проверки членов Совета директоров на предмет наличия действующей дисквалификации — запрета занимать должности в органах управления (ст. 3.11 КоАП РФ).

В проекте РДЛ задействован в бизнес-процессе [«Сложение полномочий члена Совета директоров»](business-processes/board/board-member-resignation.md) по основанию № 3 (дисквалификация, [пп. 3 п. 1 ст. 66¹ 208-ФЗ](laws/208-fz/article-66.md)).

**Текущий статус**: ручная проверка. Интеграция с API РДЛ не реализована — проверка выполняется сотрудником вручную на сайте реестра.
...
```
┌──────────────────────┐      ┌──────────────────────────┐
│  Admin Console       │      │  РДЛ ФНС                   │
│  (BoardMemberResign  │─────▶│  service.nalog.ru/         │
│   .razor)            │      │  disqualified.html         │
└──────────────────────┘      └──────────────────────────┘
         │
         │ сохраняет выписку
         ▼
...
Проверка РДЛ — ручная операция: сотрудник открывает сайт реестра, выполняет поиск, скачивает выписку и прикрепляет файл к событию сложения полномочий (`board_member_resignation`). Файл выписки сохраняется через `IFileStorage`, ссылка на него — в `user_board_member_resignations.rdl_extract_file_id`.
...
Страница: `https://service.nalog.ru/disqualified.html`

| Поле поиска | Описание |
|-------------|----------|
| Фамилия | Фамилия проверяемого лица |
| Имя | Имя проверяемого лица |
| Отчество | Отчество проверяемого лица |
| Дата рождения | Дата рождения (ДД.ММ.ГГГГ) |
| Наименование организации | Наименование организации, в которой работало лицо |
| ИНН организации | ИНН организации (для точного поиска) |

Особенности поиска:

- **Пустые поля** — формируется весь список записей реестра (не рекомендуется из-за объёма).
- **Сведения обновляются ежедневно** — актуальность данных на момент поиска гарантируется ФНС.
- **Справка об отсутствии** — если сведений об искомом лице нет в реестре, система выдаёт справку.
...
| Поле | Описание |
|------|----------|
| Номер записи РДЛ | Уникальный номер в реестре |
| Дисквалифицированное лицо | ФИО лица |
| Организация, должность | Наименование организации и должность на момент дисквалификации |
| Статья КоАП РФ | Статья, по которой вынесено решение |
| Наименование органа | Орган, составивший протокол об административном правонарушении |
| Судья | ФИО судьи, вынесшего решение |
| Сведения о дисквалификации | Дата начала и срок дисквалификации |

Поддерживается **экспорт результатов в Excel** — используется для сохранения выписки, прикрепляемой к событию сложения полномочий.

---

## Жизненный цикл проверки в проекте

```
Сотрудник (Секретарь СД / Администратор / Аудитор)
  │
  ├─ 1. Открывает https://service.nalog.ru/disqualified.html
  │
  ├─ 2. Выполняет поиск по ФИО члена СД
  │
  ├─ 3а. Результат: НЕ НАЙДЕН
  │     └─ Получает справку об отсутствии записи
  │        (процесс не инициируется — лицо не дисквалифицировано)
  │
  ├─ 3б. Результат: НАЙДЕН
  │     ├─ Скачивает выписку через «Экспорт в Excel»
  │     │
  │     ├─ 4. В Admin Console:
  │     │     └─ Инициирует событие board_member_resignation
  │     │         с типом основания DISQUALIFICATION
  │     │
  │     ├─ 5. Прикрепляет файл выписки из РДЛ (обязательно)
  │     │     └─ Сохраняется через IFileStorage
  │     │        → user_board_member_resignations.rdl_extract_file_id
  │     │
  │     └─ 6. Дата resigned_at = дата вступления в силу решения
  │           о дисквалификации (из выписки РДЛ)
  │
  └─ 7. При активной интеграции с ЭДО:
        └─ Автоматический вызов маршрута ЭДО:
           выписка + уведомление →
           председатель СД, корп. секретарь, кадровая служба
```
...
src/Domain/
  Entities/
    UserBoardMemberResignation.cs       — rdl_extract_file_id (FK → files.id)

src/Infrastructure/
  Persistence/
    ApplicationDbContext.cs             — маппинг rdl_extract_file_id

docs/
  business-processes/
    board-member-resignation.md          — бизнес-процесс, роль РДЛ (основание № 3)
```

Отдельный API-клиент для РДЛ не реализован — используется ручной веб-интерфейс ФНС.

---

## Конфигурация

РДЛ не требует конфигурации в `appsettings.json` — это внешний веб-сервис без API-ключа. При реализации автоматической интеграции (запланировано, см. раздел «API РДЛ») потребуется секция конфигурации с endpoint'ом.

---

## Роли, имеющие доступ к РДЛ-проверке

Инициировать событие `board_member_resignation` с основанием `DISQUALIFICATION` могут:

| Роль | Примечание |
|------|-----------|
| Секретарь Совета директоров | Основной исполнитель |
| Администратор системы | Резервный исполнитель |
| Участник комитета по аудиту | Не может инициировать в отношении самого себя (конфликт интересов) |
| Юрист | — |
...
ФНС предоставляет программный доступ к реестру через HTTP API по адресу `https://service.nalog.ru/disqualified.do`. На момент документирования автоматическая интеграция с API не реализована, но endpoint зафиксирован для будущей реализации.

**Ограничения API**:

- **Формат ответа**: HTML (встраиваемый фрагмент) — не JSON/XML. Требует парсинга HTML для извлечения данных.
- **Параметры запроса** (GET): `name` (ФИО), `inn` (ИНН), `ogrn` (ОГРН), `page` (страница), `pagesize` (размер страницы) и др.
- **Аутентификация**: не требуется (публичный реестр).
- **Ручной ввод капчи**: при частых запросах может потребоваться ввод капчи, что ограничивает полную автоматизацию.

При реализации автоматической интеграции рекомендуется:
- Использовать HTML-парсер (AngleSharp или HtmlAgilityPack) для извлечения структурированных данных.
- Реализовать кэширование результатов (срок действия — 24 часа, периодичность обновления реестра — ежедневно).
- Предусмотреть fallback на ручную проверку при блокировке капчей.

---

## Безопасность

- **HTTPS** — сайт ФНС доступен только по HTTPS.
- **Аутентификация** — не требуется для публичного доступа к реестру.
- **Выписка из РДЛ** — файл, сохранённый в `IFileStorage`, подпадает под WORM-семантику (не может быть удалён или изменён после публикации события). См. [Файловое хранилище](#файловое-хранилище-ifilestorage).
- **Аудит** — факт инициирования события `board_member_resignation` с основанием `DISQUALIFICATION` фиксируется в `security_audit_log` через `ISecurityAuditService`.

---

## Зависимости

- **Сайт ФНС**: `https://service.nalog.ru/disqualified.html` — публичный доступ.
- **Сетевой доступ**: HTTPS к `service.nalog.ru` (порт 443) — требуется для ручной проверки.
- **API РДЛ**: `https://service.nalog.ru/disqualified.do` — запланировано для автоматизации (не реализовано).

---

## Ссылки

- [Реестр дисквалифицированных лиц (ФНС)](https://service.nalog.ru/disqualified.html)
- [Постановление Правительства РФ от 11.11.2002 № 805](http://government.ru/docs/all/43403/)
- [Ст. 3.11 КоАП РФ — Дисквалификация](https://www.consultant.ru/document/cons_doc_LAW_34661/)
- [Ст. 66¹ 208-ФЗ — Прекращение полномочий члена СД](business-processes/board/board-member-resignation.md)
- [Бизнес-процесс: Сложение полномочий члена СД](business-processes/board/board-member-resignation.md)
...
Object Lock **включается при создании корзины** и не может быть добавлен к существующей корзине.
