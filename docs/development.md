...
Проект `src/Timeline/` — Razor Class Library, предоставляющая компоненты временной шкалы и диаграммы Ганта.

### GanttTimeline

Базовая временная шкала с календарной сеткой. Принимает `TimelineInput` (календарь, масштаб, праздники) и отображает ряды: годы, кварталы, месяцы, недели, дни недели, номера дней. Параметры `ShowQuarters`, `ShowWeeks`, `ShowDayNames`, `ShowDayNumbers`, `ShowMonths` управляют видимостью рядов.

### GanttChart

Полноценная диаграмма Ганта. Вход: `GanttChartInput` с деревом `GanttNode` (Project → Stage → Task / Milestone), календарём и списком связей `GanttDependency`.

| Параметр | Тип | По умолчанию | Назначение |
|----------|-----|-------------|------------|
| `Data` | `GanttChartInput?` | — | Входные данные |
| `RowHeight` | `int` | 32 | Высота строки |
| `TaskColor` | `string` | `#42a5f5` | Цвет бара задачи |
| `StageColor` | `string` | `#78909c` | Цвет бара этапа/проекта |
| `MilestoneColor` | `string` | `#9c27b0` | Цвет вехи |
| `OverdueColor` | `string` | `#d32f2f` | Цвет просроченной задачи |
| `ProgressColor` | `string` | `rgba(0,0,0,0.15)` | Цвет прогресса в баре |

**Масштабы**: День (30px), Неделя (20px), Месяц (3px), Квартал (3px), Год (4px).

**Связи**: `GanttDependency` — от задачи-предшественника к последователю. Типы: `Regular`, `CriticalPath`, `Warning`. Настройка: цвет, стиль (solid/dashed), толщина. Отрисовка SVG-линиями.

**Структура левой панели**: кнопки масштаба, «Настройка» (модальное окно с чекбоксами видимости рядов), «Сегодня». Высота синхронизируется с заголовком шкалы.

### Интеграция

```razor
<GanttChart Data="@input" RowHeight="32"
            TaskColor="#42a5f5" StageColor="#78909c" />
```

Страницы, использующие компонент:
- `/gantt-test` — тестовая страница с демо-данными
- `/org-plan` — план организационных мероприятий (интенты → проекты, этапы → этапы, задачи → задачи)
