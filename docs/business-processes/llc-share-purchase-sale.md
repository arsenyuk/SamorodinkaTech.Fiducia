...
- Нотариус проверяет: оплату доли, отсутствие арестов/залогов, полномочия сторон, согласие супруга (если требуется).
...
### 10. Алгоритм продажи доли обществу ([ст. 23](../laws/article-14fz-23.md), [ст. 26](../laws/article-14fz-26.md) 14-ФЗ)

```mermaid
flowchart TD
    A["Начало: Участник хочет продать долю ООО"] --> B{Проверка устава}
    B -->|"Запрет продажи третьим лицам"| C["Подача заявления о выходе"]
    C --> D["Доля переходит к обществу"]
    D --> E["ООО выплачивает действительную стоимость"]
    B -->|"Продажа ООО разрешена"| F["Участник направляет оферту ООО"]
    F --> G["Нотариальное удостоверение оферты"]
    G --> H["ООО принимает решение о покупке"]
    H --> I["Заключение договора купли-продажи"]
    I --> J["Нотариальное удостоверение договора"]
    J --> K["Регистрация в ЕГРЮЛ"]
    K --> L["Доля стала казначейской"]
    L --> M{Реализация в течение 1 года}
    M -->|Продажа| N["Конец"]
    M -->|Распределение| N
    M -->|Погашение| N
    M -->|"Пропуск срока"| O["Риск судебного иска"]
```

1. Участник проверяет устав: разрешена ли продажа доли самому ООО.
2. Если устав запрещает продажу третьим лицам, участник подаёт нотариально удостоверенное заявление о выходе из ООО – доля переходит к обществу, и оно обязано выплатить действительную стоимость.
3. Если продажа обществу разрешена, участник направляет в ООО нотариально удостоверенную оферту о продаже доли.
4. Общее собрание участников принимает решение о покупке доли (или отказе).
5. При положительном решении стороны заключают договор купли-продажи.
6. Договор подлежит нотариальному удостоверению (если это не выход, а продажа).
7. Нотариус подаёт документы на регистрацию изменений в ЕГРЮЛ.
8. Доля переходит к обществу и становится казначейской.
9. Общество обязано в течение одного года продать, распределить или погасить эту долю. При пропуске срока – риск судебного иска от бывшего участника.
...
### 11. Алгоритм продажи доли третьим лицам ([ст. 21](../laws/article-14fz-21.md) 14-ФЗ)
...
flowchart TD
    ...
    N --> O{Нотариус удостоверил?}
    O -->|Нет| P["❌ Сделка ничтожна"]
    O -->|Да| Q["Нотариус проверил (оплата, аресты, полномочия)"]
    Q -->|Проблемы| R["❌ Отказ в удостоверении"]
    Q -->|ОК| S["Регистрация в ЕГРЮЛ"]
    S --> T{Сделка оспорена?}
    T -->|Да| U["❌ Суд признал недействительной – покупатель теряет всё"]
    T -->|Нет| V["✅ Сделка завершена, покупатель – участник"]
```
...
- Сделка может быть оспорена в суде по основаниям: нарушение преимущественного права, отсутствие согласия супруга, арест доли, банкротство продавца, неодобрение сделки с заинтересованностью. В этом случае покупатель теряет и долю, и деньги.

---

### 12. Алгоритм покупки доли обществом ([ст. 23](../laws/article-14fz-23.md), [ст. 26](../laws/article-14fz-26.md) 14-ФЗ)

```mermaid
flowchart TD
    A["Участник владеет долей"] --> B{Есть основание для обязательного выкупа?}
    B -->|Да| C["Участник направляет нотариальное требование о выкупе"]
    ...
    C --> I{"Срок требования соблюдён? (45 дней)"}
    I -->|Нет| J["❌ Общество освобождено от выкупа"]
    I -->|Да| K["Общество обязано выкупить"]
    G --> K
    K --> L["Расчёт действительной стоимости"]
    L --> M{Согласие участника?}
...
3. Если срок пропущен – общество освобождается от обязанности выкупа.
...
### 13. Алгоритм покупки доли третьим лицом ([ст. 21](../laws/article-14fz-21.md) 14-ФЗ)
...
flowchart TD
    ...
    M --> N{Нотариус удостоверил?}
    N -->|Нет| O["❌ Сделка ничтожна"]
    N -->|Да| P["Нотариус проверил оплату, аресты, согласие супруга"]
    P -->|Проблемы| Q["❌ Отказ в удостоверении"]
    P -->|ОК| R["Регистрация в ЕГРЮЛ – покупатель стал участником"]
    R --> S{Сделка оспорена?}
    S -->|Да| T["❌ Суд признал недействительной – покупатель теряет деньги и долю"]
    S -->|Нет| U["✅ Покупка успешна, права защищены"]
```
...
13. Сделка может быть оспорена в суде (нарушение преимущественного права, отсутствие согласия супруга, арест, банкротство продавца и др.) – тогда покупатель теряет и долю, и деньги.
