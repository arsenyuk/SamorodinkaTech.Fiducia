...
flowchart TD
    ...
    T --> U{Сделка оспорена?}
    U -->|Да| V["❌ Суд признал недействительной – покупатель теряет всё"]
    U -->|Нет| W["✅ Сделка завершена, покупатель – участник"]
```
...
flowchart TD
    ...
    S --> T{Сделка оспорена?}
    T -->|Да| U["❌ Суд признал недействительной – покупатель теряет деньги и долю"]
    T -->|Нет| V["✅ Покупка успешна, права защищены"]
```
