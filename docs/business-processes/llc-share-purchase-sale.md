...
flowchart TD
    ...
    Q -->|ОК| O{Нотариус удостоверил сделку?}
    O -->|Нет| P["❌ Сделка ничтожна"]
    O -->|Да| S["Нотариус регистрирует переход доли в ЕГРЮЛ"]
    S --> T{Сделка оспорена?}
...
flowchart TD
    ...
    P -->|ОК| N{Нотариус удостоверил сделку?}
    N -->|Нет| O["❌ Сделка ничтожна"]
    N -->|Да| R["Нотариус регистрирует переход доли в ЕГРЮЛ – покупатель стал участником"]
    R --> S{Сделка оспорена?}
