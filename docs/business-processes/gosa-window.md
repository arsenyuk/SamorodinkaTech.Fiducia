...
flowchart TD
    ...
    B -->| ООО| L[ООО]
    L --> P{Уставом предусмотрен<br>совет директоров?<br>ст. 32 14-ФЗ}
    P -->|Да| M[ООСУ определяет СД<br>интервал фиксирован<br>ст. 34 14-ФЗ]
    P -->|Нет| Q[ООСУ определяют<br>участники ООО]
    M --> N[Применить фиксированный<br>интервал 01.03–30.04]
    Q --> N
    ...
    P@{ shape: diamond, label: "Уставом предусмотрен\nсовет директоров?\nст. 32 14-ФЗ" }
    M@{ shape: rounded, label: "ООСУ определяет СД\nинтервал фиксирован\nст. 34 14-ФЗ" }
    Q@{ shape: rect, label: "ООСУ определяют\nучастники ООО" }
    N@{ shape: rect, label: "Применить фиксированный\nинтервал 01.03–30.04" }
