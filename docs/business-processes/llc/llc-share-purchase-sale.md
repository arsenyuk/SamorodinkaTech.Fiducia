...
- Основной закон: № 14‑ФЗ «Об обществах с ограниченной ответственностью», [статья 21](../laws/14-fz/article-14fz-21.md).
- Подробно об ограничениях, которые может содержать устав ООО — [отдельная статья](llc-charter-restrictions.md).
- Поправки 2025 года:
...
3. **Проверяет факт брака через ЕГР ЗАГС** — с 2021 года нотариусы имеют доступ к [Единому реестру записей актов гражданского состояния](../laws/article-zags.md) через СМЭВ и могут проверить запись о браке онлайн, исключая подделку свидетельства.
4. **Проверяет содержание и срок действия согласия** — в согласии должны быть указаны данные супруга-продавца, данные супруга-соглашающегося, предмет согласия (конкретная доля в конкретном ООО). Размытые формулировки («согласен на любые сделки») не принимаются. Также проверяется, не истёк ли срок действия согласия (обычно 1 год или до совершения конкретной сделки).
5. **Проверяет подлинность нотариального акта** — если согласие выдано другим нотариусом, подлинность проверяется через [Единую информационную систему нотариата](../laws/article-notary-eis.md) (ЕИС). Нотариус, выдавший согласие, уже удостоверил личность супруга — повторная сверка паспорта не требуется.
...
flowchart TD
    ...
    L -->|Да| M["Проверка содержания, срока и подлинности через ЕИС"]
    M --> N{Согласие действительно?}
    N -->|"Нет (размыто, истёк срок, не подтверждено в ЕИС)"| R6["❌ Отказ в удостоверении: согласие недействительно"]
    N -->|Да| CERT
    CERT --> Q["Нотариус подаёт заявление в ФНС (ЕГРЮЛ) в течение 2 рабочих дней"]
```

#### Схема взаимодействия нотариуса с информационными системами

Каждое действие с долей регистрируется нотариусом в нескольких системах. На схеме показано, какие действия нотариуса с какими системами связаны.

```mermaid
flowchart LR
    N["Нотариус"]
    
    subgraph UC["Действия с долями"]
        U1["U1: Удостоверить сделку купли-продажи доли"]
        U2["U2: Удостоверить оферту о продаже доли"]
        U3["U3: Удостоверить заявление о выходе участника"]
        U4["U4: Удостоверить согласие супруга на продажу"]
        U5["U5: Выдать выписку из реестра списков участников"]
        U6["U6: Зарегистрировать изменения в ЕГРЮЛ"]
        U7["U7: Проверить семейное положение продавца"]
    end
    
    subgraph IS["Информационные системы"]
        EIS["ЕИС нотариата (реестр списков участников ООО)"]
        EGRUL["ЕГРЮЛ (ФНС)"]
        ZAGS["ЕГР ЗАГС (ФНС)"]
        SMEV["СМЭВ (шлюз межведомственного взаимодействия)"]
    end
    ...
    U6 -.-> SMEV
    U7 -.-> SMEV
    SMEV -.->|"U1, U3, U6: рег. в ЕГРЮЛ"| EGRUL
    SMEV -.->|"U7: запрос в ЗАГС"| ZAGS
```

| Действие нотариуса | ЕИС нотариата | ЕГРЮЛ (ФНС) | ЕГР ЗАГС (через СМЭВ) |
|--------------------|:---:|:---:|:---:|
| Удостоверить сделку купли-продажи доли | ✓ обновляет реестр участников | ✓ переход доли регистрируется | — |
| Удостоверить оферту о продаже доли | ✓ регистрирует факт оферты | — | — |
| Удостоверить заявление о выходе участника | ✓ обновляет реестр (казначейская доля) | ✓ изменение состава участников | — |
| Удостоверить согласие супруга на продажу | ✓ регистрирует согласие | — | — |
| Выдать выписку из реестра списков участников | ✓ читает реестр, формирует выписку | — | — |
| Проверить семейное положение продавца | — | — | ✓ запрос через СМЭВ |

> **Ключевое:** все нотариальные действия с долями, кроме выдачи выписки, оставляют след в ЕИС. ЕГРЮЛ обновляется только при действиях, меняющих состав участников (купля-продажа, выход, наследование). ЕГР ЗАГС — только для проверки семейного положения. СМЭВ — транспортный шлюз, нотариус напрямую с ЕГР ЗАГС и ФНС не работает.

### 5. Казначейская доля ([ст. 24](../laws/14-fz/article-14fz-24.md) 14-ФЗ)
...
- **Отдельный бизнес-процесс:** [Переход неоплаченной доли к обществу](unpaid-share-transfer-to-society.md) — порядок действий при неоплате доли участником.

### 6. Определение стоимости доли ([ст. 23](../laws/14-fz/article-14fz-23.md) 14-ФЗ)
...
> Полный перечень вопросов компетенции ОСУ, включая таблицу кворумов — в [отдельной статье](osu-ooo-questions.md).
...
По общему правилу казначейская доля продаётся по цене **не ниже** той, которая была уплачена обществом при её приобретении. Однако **общее собрание участников вправе установить иную цену** — своим решением, принятым единогласно ([п. 4 ст. 24](../laws/14-fz/article-14fz-24.md) 14-ФЗ).

#### 10.3. Распределение казначейской доли между участниками

Вместо продажи общество может **распределить** казначейскую долю между всеми участниками пропорционально их существующим долям. Это также делается **по решению общего собрания участников** ([п. 1 ст. 24](../laws/14-fz/article-14fz-24.md) 14-ФЗ).

#### 10.4. Изменение устава, касающееся порядка продажи долей

Любые изменения в уставе, которые затрагивают правила продажи долей (например, введение или отмена запрета на продажу третьим лицам, установление требования о согласии участников, отмена преимущественного права), принимаются **общим собранием участников** ([п. 4.1 ст. 21](../laws/14-fz/article-14fz-21.md) 14-ФЗ).
...
### 11. Алгоритм продажи доли обществу ([ст. 23](../laws/14-fz/article-14fz-23.md), [ст. 26](../laws/14-fz/article-14fz-26.md) 14-ФЗ)
...
### 12. Алгоритм продажи доли третьим лицам ([ст. 21](../laws/14-fz/article-14fz-21.md) 14-ФЗ)

```mermaid
flowchart TD
    A["Участник решил продать долю третьему лицу"] --> B{Устав разрешает?}
    B -->|Нет| C["❌ Продажа невозможна, только выход"]
    B -->|Да| D{Требуется согласие участников или ОСУ?}
...
3. Если требуется согласие участников или ОСУ – продавец его запрашивает. Без согласия сделка блокируется.
...
### 13. Алгоритм покупки доли обществом ([ст. 23](../laws/14-fz/article-14fz-23.md), [ст. 26](../laws/14-fz/article-14fz-26.md) 14-ФЗ)
...
### 14. Алгоритм покупки доли третьим лицом ([ст. 21](../laws/14-fz/article-14fz-21.md) 14-ФЗ)
...
flowchart TD
    ...
    C -->|Да| E{Требуется согласие участников или ОСУ?}
...
4. Если требуется согласие участников или ОСУ – продавец должен его получить; без этого сделка блокируется.
