...
| `SECRETARY` | Секретарь | Организация заседаний, документы |
...
Конфигурация логирования — исключительно в `appsettings.json` / переменных окружения (см. AGENTS.md). Изменение через UI не предусмотрено.
